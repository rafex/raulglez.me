  ...
  connectedCallback(): void {
    ...
    shadow.innerHTML = `
      <style>
        ...
        .skill__badge { display: inline-block; background: var(--color-bg-alt); border: 1px solid var(--color-border); border-radius: 20px; padding: var(--spacing-xs) var(--spacing-md); font-size: var(--font-size-sm); color: var(--color-text); transition: all var(--transition-fast); }
      ...
      <section class="cv__section" data-animate="fadeInUp">
        <div class="section__container">
          ...
          <div class="skills__grid">${(data.technical || []).map(s => `<span class="skill__badge" data-animate="zoomIn">${s}</span>`).join('')}</div>
          <h2 class="section__title skills__subtitle">Competencias</h2>
          <div class="skills__grid skills__grid--soft">${(data.competencies || []).map(c => `<span class="skill__badge skill__badge--soft" data-animate="zoomIn">${c}</span>`).join('')}</div>
        </div>
      </section>`;
  }
}
